...
@interface LSBqApplyViewController ()<NIDropDownDelegate>
@property (weak, nonatomic) IBOutlet UIView *detailView;
@property (weak, nonatomic) IBOutlet UITextView *bqTextView;
...
@implementation LSBqApplyViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title = @"诉讼保全申请";
    ...
    self.bqTextView.layer.cornerRadius = 5;
    self.bqTextView.layer.borderWidth = 1;
    self.bqTextView.layer.borderColor = LSGrayColor.CGColor;
    [self loadAhInfo];
...
- (void)handleKeyboardDidShow:(NSNotification*)paramNotification
{
    ...
    self.bqTextView.contentInset=UIEdgeInsetsMake(0, 0,keyboardRect.size.height, 0);
}

- (void)handleKeyboardDidHidden
{
    self.bqTextView.contentInset=UIEdgeInsetsZero;
}
...
{
    [self.bqTextView  resignFirstResponder];
}
...
- (IBAction)zanCunBtn:(id)sender {
    if (!self.ajbsStr || self.bqTextView.text.length == 0) {
    ...
    params[@"sqyy"] = self.bqTextView.text;
