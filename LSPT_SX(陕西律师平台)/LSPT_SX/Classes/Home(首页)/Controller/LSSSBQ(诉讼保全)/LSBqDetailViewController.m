...
@interface LSBqDetailViewController ()<LSDropdownMenuDelegate>
@property (weak, nonatomic) IBOutlet UIView *detailView;
@property (weak, nonatomic) IBOutlet UILabel *ahqcLabel;
@property (weak, nonatomic) IBOutlet UILabel *cbfgLabel;
@property (weak, nonatomic) IBOutlet UILabel *twbtlabel;
@property (weak, nonatomic) IBOutlet UILabel *nrLabel;
@property (nonatomic, strong) LSDropdownMenu *menu;
@end
NSString *ssbqAhqcStr;
NSString *ssbqIdStr;
NSString *ssbqSqyyStr;
@implementation LSBqDetailViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title = @"诉讼保全详情";
    ...
    if ([_ssbqDetail.zt isEqualToString:@"1"]) {
    ...
    } else if ([_ssbqDetail.zt isEqualToString:@"3"]) {
    ...
    } else if ([_ssbqDetail.zt isEqualToString:@"4"]) {
    ...
    _ahqcLabel.text = _ssbqDetail.ahqc;
    if ([_ssbqDetail.spsj isEqualToString:@"0"]) {
        _cbfgLabel.text = @" ";
    } else {
        _cbfgLabel.text = [self covertToDateStringFromString:_ssbqDetail.spsj];
    }
    _twbtlabel.text = _ssbqDetail.spyj;
    _nrLabel.text = _ssbqDetail.sqyy;
    
    ssbqAhqcStr = _ssbqDetail.ahqc;
    ssbqIdStr = _ssbqDetail.ID;
    ssbqSqyyStr = _ssbqDetail.sqyy;
...
{
    NSLog(@"修改");
    LSBqChangeViewController *changeVC = [[LSBqChangeViewController alloc]init];
...
- (void)deleteDidClick
{
    ...
    param[@"id"] = _ssbqDetail.ID;
    [LSHttpTool get:SSBQDelUrl params:param success:^(id json) {
        NSLog(@"==%@",json);
